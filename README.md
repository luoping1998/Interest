...
- 前端
1）查看：主要是通过点击帖子进入帖子详情页面，每个贴子在被渲染的时候拥有自己的帖子id，当帖子的最外层（内部cilick事件均取消了冒泡）监听到点击事件时，会向服务器发送请求，获取帖子的详情信息并进入详情页面进行展示。同理，对评论的详情查看也是这个思路。
2）点赞：首先会检测是否登录，登录状态存储在store中，如若未登录则弹出提示框，并跳转至登录页面；如若登录，则会将用户id,以及帖子id作为参数传参给后台，后台进行检测，返回相应的mes：已点赞->点赞失败或点赞成功。
3）转发：在验证了登录状态后，将原贴的信息如一存储在数据库中，origin存储为原贴作者id，类型为1，表示为转发。前端展示帖子时根据数据中类型不同，稍微展示有区别。
4）评论：前端检测评论是否合法，合法则将用户id，帖子id作为参数发起请求，后台收到请求后存储数据库，并给以合理的响应
	-  回复：同样检测评论是否合法，合法后则将用户id，评论id作为参数发起请求，后台收到请求后存储数据库，给以响应
...
#####  思路：
- 前端：
前端监听keydowm事件，每一次都向后台获取此时value的相关数据，并渲染页面
- 后台：
根据请求的参数，查询数据库，返回相应的数据
#### 6、接收推送
##### 思路
使用的是SSE服务器发送事件，这里没有使用websocket的原因是在于考虑到前端只需要发送一次请求，后续状态均只需等待服务器向前端发送数据即可，而websocket多用于双向数据传递。[SSE服务器发送事件](https://github.com/luoping1998/Interest/wiki/SSE%28Server-Sent-Events-%E6%9C%8D%E5%8A%A1%E5%99%A8%E5%8F%91%E9%80%81%E4%BA%8B%E4%BB%B6%29)
- 前端：
在登录之后向服务器发起一次请求，该请求一直处于等待状态。
- 后台：
在第一次收到请求后便开始轮询服务器，没错，就是服务器设定了一个定时器 ，每个循环搜索一次数据库看是否有未查看的通知，如果有则发给前端，如果没有就继续循环，直到有。
- 数据库：
数据库中会存储相关的“通知”，即在每次有关注、评论、转发、回复的请求时，便会存储一条通知到数据库，并设定初始状态为0,即未读，在用户点击通知按钮进入通知详情页面时再向后台发送请求，代表通知已读。然后后台会在数据库中修改该用户相关的所有通知的状态。
#### 7、其他
- 世界下拉加载、上拉刷新功能
思路：
检测第一次mousedown的top位置和最后mousemove的top位置，通过检测两者之间的距离大小，判断是否为下拉加载（上拉刷新），若是：下拉加载则将当前条数count作为参数传给后台，后台获取最新帖子，截取count ~ count+10条返回给前端，前端接收后push进当前展示的数组中，页面自动重新渲染；上拉刷新则是检测两次差值后判断为上拉状态后，向后台请求0~10范围的最新帖子，替代当前展示数组，页面渲染。
### 待改进地方
1. 后台传出图片的操作有点蠢，最好的办法是使用url路径，而不是传输二进制的文件
2. 上传头像的部分没有裁剪功能
3. 由于部分导航栏为fixed布局，设定的width为100%，所以电脑端打开进入世界以及某些页面时略显奇怪，特别是 ，弹窗弹出的时候。
4. 代码复用部分较少，命名格式不够规范
