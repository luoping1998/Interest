...
var login = function( db, email, name,callback) {
	...
	var sem = 'SELECT `id`,`email`,`u_name`,`u_level`,`starnum`,`fansnum`,`signature` , `msgnum` ,`path`,`sex`,`show` FROM `usertable` WHERE `email` = \'' + email +'\'';
	var sna = 'SELECT `id`,`email`,`u_name`,`u_level`,`starnum`,`fansnum` ,`signature` , `msgnum` , `path` ,`sex`,`show` FROM `usertable` WHERE `u_name` = \'' + name +'\'';
